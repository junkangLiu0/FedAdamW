...
class DataWorker(object):
    ...
    def update_FedLADA(self, weights, E, index, momen_m, momen_v, lr, step):
        ...
        for k, v in self.model.named_parameters():
            momen_m[k] = momen_m[k].to(device)
        args.gamma=0.9
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=lr*(1-args.gamma), weight_decay=0.001)
        for group in self.optimizer.param_groups:
            for p in group['params']:
                if p.grad is None:
                    continue
    ...
    def update_func(self, alg, weights, E, index, lr, ps_c=None, v=None, step=None, shared_state=None,ci=None):
        self.load_dict()
        if alg in {'FedCM','FedACG','IGFL','FedNSAM','FedACG','FedNesterov'}:
            return self.func_dict.get(alg, None)(weights, E, index, ps_c, lr)
        if alg in {'FedLADA','FedAdamW','FedMuon','FedLADA'}:
...
if __name__ == "__main__":
    ...
    if args.CNN == 'VIT-B':
        if args.weights != "":
            ...
            weights_dict = torch.load('./model_weight/vit_base_patch16_224_in21k.pth', map_location=device)
            # 删除不需要的权重
    ...
    if args.CNN == 'VIT-L':
        if args.weights != "":
            assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
            weights_dict = torch.load('./model_weight/jx_vit_large_patch16_224_in21k-606da67d.pth', map_location=device)
            # 删除不需要的权重
            del_keys = ['head.weight', 'head.bias'] if model.has_logits \
                else ['pre_logits.fc.weight', 'pre_logits.fc.bias', 'head.weight', 'head.bias']
            for k in del_keys:
    ...
    if args.CNN == 'swin_tiny' and args.pre == 1:
        if args.weights != "":
            assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
            weights_dict = torch.load('./model_weight/swin_tiny_patch4_window7_224.pth', map_location=device)["model"]
            # 删除有关分类类别的权重
    ...
    if args.CNN == 'swin_small':
        if args.weights != "":
            assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
            weights_dict = torch.load('./model_weight/swin_small_patch4_window7_224.pth', map_location=device)["model"]
            # 删除有关分类类别的权重
            for k in list(weights_dict.keys()):
                if "head" in k:
                    del weights_dict[k]
    ...
    if args.CNN == 'swin_base':
        ...
        if args.weights != "":
            assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
            weights_dict = torch.load('./model_weight/swin_base_patch4_window7_224_22k.pth', map_location=device)["model"]
            # 删除有关分类类别的权重
            for k in list(weights_dict.keys()):
                if "head" in k:
                    del weights_dict[k]
    ...
    if args.CNN == 'swin_large':
        if args.weights != "":
            assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
            weights_dict = torch.load('./model_weight/swin_large_patch4_window7_224_22k.pth', map_location=device)["model"]
            # 删除有关分类类别的权重
            for k in list(weights_dict.keys()):
                if "head" in k:
                    del weights_dict[k]
    ...
    for epochidx in range(epoch_s, epoch):
        ...
        elif alg in {'FedAvg', 'FedSWA','Fedprox', 'FedAvg_adamw', 'FedAvg_adam','FedSAM','Local_Soap','Local_Muon','Local_Shampoo','Local_AdEMAMix'}:
            weights = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
